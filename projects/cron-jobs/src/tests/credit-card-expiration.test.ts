...
describe("CreditCardExpiration Job", () => {
  beforeAll(async () => {
    ...
    const creditCard1: any = await createCreditCard('ACTIVE', DateTime.now().plus({month: 8}).toJSDate());
    creditCards.push(creditCard1);
    // 1 Credit Card status="Expired"
    const creditCard2: any = await createCreditCard('EXPIRED', DateTime.now().minus({month: 8}).toJSDate());
    creditCards.push(creditCard2);
    // 1 Credit Card status="Active" but with an
    const creditCard3: any = await createCreditCard('ACTIVE', DateTime.now().minus({month: 8}).toJSDate());
  ...
  test("Set status `EXPIRED` when the expiration is older than today", async () => {
    ...
    dbConnection = await mongoose.connect(process.env.MONGO_DB_URI || '');
	  CreditCardModel = mongoose.model('CreditCard', CreditCardSchema);
    const creditCard1Updated: any =  await CreditCardModel.findOne({_id: creditCards[0]._id});
    expect(creditCard1Updated.status).toEqual('ACTIVE');
    const creditCard2Updated: any =  await CreditCardModel.findOne({_id: creditCards[1]._id});
    expect(creditCard2Updated.status).toEqual('EXPIRED');
    const creditCard3Updated: any =  await CreditCardModel.findOne({_id: creditCards[2]._id});

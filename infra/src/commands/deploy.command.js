...
const deployCommand = async (options) => {
    ...
    try {
        execSync(`RAILWAY_TOKEN=${projectAuthToken} railway up -s ${serviceName} -e ${environmentName}  -d`).toString("utf8");
    } catch {
    ...
    console.log(`Service ${serviceName} successfully deployed into ${environmentName}`)
}

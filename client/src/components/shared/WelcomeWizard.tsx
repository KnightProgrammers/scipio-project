...
import { Container, CustomControl, CustomSelectOption, Loading } from "@/components/shared";
...
import { langOptions, LanguageOption } from "@/views/account/Settings/components/Profile";
    ...
    <Container className="flex flex-auto flex-col h-[100vh] justify-center items-center">
      <Card className="p-4 min-w-[280px]">
        <Steps current={step} className="w-full">
          ...
          <Steps.Item customIcon={step === STEPS.SAVING ? <Spinner/> : <MdOutlineRocketLaunch/>} />
        </Steps>
        <div className="mt-6 h-80 rounded flex items-center justify-center">
          {
            step === STEPS.START && <div className="text-center">
          ...
          {
            step === STEPS.LANG && <div>
              <h2 className="mb-4 text-center">{t('pages.welcome.headers.lang')}</h2>
          ...
          {
            step === STEPS.COUNTRY && <div>
              <h2 className="mb-4 text-center">{t('pages.welcome.headers.country')}</h2>
              <Select
          ...
          {
            step === STEPS.SAVING && isSaving && <div>
              <Loading loading />
        ...
        <div className="mt-4 text-center">
          {
            (step != STEPS.COUNTRY || !!selectedCountry) && step != STEPS.SAVING && <Button
              disabled={step === STEPS.SAVING || (step === STEPS.COUNTRY && !selectedCountry)}
              variant="solid" onClick={onNext}
              className="w-full">
              {t('actions.next')}
            </Button>
          }

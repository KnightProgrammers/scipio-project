...
const SignInForm = (props: SignInFormProps) => {
  ...
  const onSignInWithGoogle = async () => {
    try {
      ...
      setMessage(t(`error.auth.firebase.${e.code}`) || 'Error with Google')
    }
  }
  const onSignInWithPassword = async (values: SignInFormSchema, setSubmitting: (isSubmitting: boolean) => void) => {
    try {
  ...
  return (
    <div className={className}>
      ...
      <div className="mb-4">
        <Button onClick={() => onSignInWithGoogle()} icon={<FcGoogle/>} loading={loadingWithProvider}>
